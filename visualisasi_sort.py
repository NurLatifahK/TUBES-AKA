...
root = Tk()
root.title('Aplikasi Visualisasi Insertion Sort & Heap Sort')
...
def insertionSort(arr,arrayVisual,canvas):
    global time_start
    time_start = time.time()
    ...
    for i in range(0, len(arr)):
        ...
        while j >= 0 and key < arr[j]:
            arr[j+1] = arr[j]
            arrayVisual(arr, ['green' if x == j or x == j+1 else 'red' for x in range(len(arr))], canvas)
...
def run():
    ...
    insertionSort(arr2, arrayVisual, canvas2)
...
Label(frameText2,text="Insertion Sort\n\nRunning Time (detik):\n\n",bg='white').grid(row=2,column=0)
